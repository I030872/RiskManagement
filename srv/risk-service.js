...
module.exports = cds.service.impl(async function () {
     ...
     * Each requst to the API Business Hub requires the apikey in the header.
     */
    this.on("READ", BusinessPartners, async (req) => {
        //The API Sanbox returns alot of business partners with empty names.
        //We don't want them in our applicaiton 
    ...
    /**
     * Event-handler for risks.
     * Retrieve BusinessPartner data from the external API.
     */
    this.on("READ", Risks, async (req, next) => {
        /* 
        Check whether the request wants an "expand" of the business partner At this is not possible, the risk entity and the business partner entity
            ...
            await Promise.all( 
                res.map(async (risk) => {
                    const bp = await BPsrv.transaction(req).send({
                        query: SELECT.one(this.entities.BusinessPartners).where({ BusinessPartners: risk.bp_BusinessPartner })
